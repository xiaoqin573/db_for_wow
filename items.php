    ...
    if($_GET['items'] == '0') {
     ...
     // Составляем запрос к БД, выполняющий поиск по заданным классу и подклассу
        $total = $DB->select('
            ...
            ',  ($_SESSION['locale']) ? $_SESSION['locale'] : DBSIMPLE_SKIP, ($_SESSION['locale']) ? 1 : DBSIMPLE_SKIP, ($class != '') ? $class : DBSIMPLE_SKIP, (1) ? ' 0,1,2,3,4,5,6,-3,7,8 ' : DBSIMPLE_SKIP
        );
        ...
        $rows = $DB->select('
            ...
            ', $item_cols[2], ($_SESSION['locale']) ? $_SESSION['locale'] : DBSIMPLE_SKIP, ($_SESSION['locale']) ? 1 : DBSIMPLE_SKIP, ($class != '') ? $class : DBSIMPLE_SKIP, (1) ? ' 0,1,2,3,4,5,6,-3,7,8 ' : DBSIMPLE_SKIP
        );
    }else {
           ...
           $total = $DB->select('
            ...
            ', ($_SESSION['locale']) ? $_SESSION['locale'] : DBSIMPLE_SKIP, ($_SESSION['locale']) ? 1 : DBSIMPLE_SKIP, ($class != '') ? $class : DBSIMPLE_SKIP, ($subclass != '') ? $subclass : DBSIMPLE_SKIP
        );
