...
function render_spell_tooltip(&$row) {
    ...
    $name=$row['spellname_loc' . $_SESSION['locale']];
    $x .= '<b>' .($row['chinese'] ? $row['chinese'] : $name). '（'.$name. '）' . '</b><br />';
...
function allspellsinfo2(&$row, $level=0) {
    ...
    if ($level > 0) {
        ...
        $allspells[$num]['chinese'] = $row['chinese'] ? $row['chinese'] : $row['name'];
...
function spellinfo2(&$row) {
    ...
    if ($row) {
        ...
        $spell['chinese'] =  $row['chinese'] ?  $row['chinese'] :  $row['name'];
