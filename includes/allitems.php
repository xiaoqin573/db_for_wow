...
function allitemsinfo2(&$Row, $level=0) {
    ...
    // Ищем иконку
    $allitems[$num]['icon'] = strtolower($Row['iconname']);
    // Качество вещи
    if ($Row['quality'] == 7)
        $Row['quality'] = 6;
    ...
    $allitems[$num]['chinese'] = $Row['chinese'] ? $Row['chinese'] : $Row['name'];
...
function getitemname($id) {
    global $DB;
    $z = $DB->selectRow('
			SELECT chinese,name {, l.name_loc?d as `name_loc`}
    ...
    return $z['chinese'] ? $z['chinese'] : (!empty($z['name_loc']) ? $z['name_loc'] : $z['name'] );
}
...
function render_item_tooltip(&$Row) {
    ...
    $x .= '<b class="q' . $Row['quality'] . '">' .($Row['chinese'] ? $Row['chinese'] : $name) .'（'.$name.'）' . '</b>';
    // Биндинг вещи
    $x .= $bond[$Row['bonding']];
...
function iteminfo2(&$Row, $level=0) {
    ...
    $item['chinese'] = !empty($Row['chinese']) ? $Row['chinese'] : $Row['name'];
    // Тип вещи
