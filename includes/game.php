...
function mass_coord(&$data) {
    ...
    foreach ($data as $ndata) {
        ...
        if (isset($ndata['spawntimesecs']))
            $tmp = array_merge(coord_db2wow($ndata['m'], $ndata['x'], $ndata['y'], false), array('r' => sec_to_time($ndata['spawntimesecs'])) ?: array());
        else
...
function factioninfo($id) {
    ...
    $faction['chinese'] = $DB->selectCell('SELECT chinese FROM ?_aowow_factions WHERE factionID = ?d LIMIT 1', $id);
    $faction['entry'] = $id;
    return $faction;
}
