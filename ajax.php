...
switch ($what) {
    case 'item':
        ...
        if ($item['name'])
            $x .= 'name: \'' . str_normalize($item['chinese']).'（'.str_normalize($item['name']).'）' . '\',';
        if ($item['quality'])
            $x .= 'quality: ' . $item['quality'] . ',';
        if ($item['icon']){
            $item['icon'] = strtolower(str_replace("\r",'',$item['icon']));
    ...
    case 'spell':
        ...
        if ($spell['name'])
            $x .= 'name: \'' . str_normalize($spell['chinese']). str_normalize($spell['name']) . '\',';
        if ($spell['icon']){
            $spell['icon'] = strtolower(str_replace("\r",'',$spell['icon']));
    ...
    case 'quest':
        ...
        if ($quest['name'])
            $x .= 'name: \'' . str_normalize($item['chinese']).'（'.str_normalize($quest['name']).'）' . '\',';
        if ($quest['tooltip'])
