...
array(
    'title' => array('.quest-title a  ','text','',function($content){
        return $content;
    }),
...
foreach($data as $key=>$temp){
    $sql .= "UPDATE `wow`.`quest_template` SET `chinese` = \"".addslashes($temp['title'])."\" WHERE `quest_template`.`entry` = \"".$temp['id']."\"; \r\n"; 
}
...
?>
<script type="text/javascript">setTimeout(function() { window.location.href='http://localhost:8888/wowhead/tool/QueryList/quests.php?page=<?php echo ($intPage+1);?>'; }, 1000);</script>
