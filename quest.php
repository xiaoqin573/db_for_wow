...
if (1) {
    ...
    while ($tmp) {
        $tmp = $DB->selectRow('
			SELECT q.entry, q.Title, q.chinese
        ...
        if ($tmp) {
            ...
            if (!empty($tmp['chinese']))
                $tmp['chinese'] = $tmp['chinese'];
            else
                $tmp['chinese'] = $tmp['Title'];
    ...
    while ($tmp) {
        $tmp = $DB->selectRow('
			SELECT q.entry, q.Title, q.chinese, q.NextQuestInChain
				{, l.Title_loc?d AS `Title_loc`}
			FROM ?_quest_template q
    ...
    if (!$quest['req'] = $DB->select('
				SELECT q.entry, q.chinese as Title, q.NextQuestInChain
					{, l.Title_loc?d AS `Title_loc`}
				FROM ?_quest_template q
					{LEFT JOIN (?_locales_quest l) ON l.entry=q.entry AND ?}
				WHERE
    ...
    // НПС
    $rows = $DB->select('
		SELECT c.entry, c.name, A, H,chinese
